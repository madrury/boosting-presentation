...
\begin{frame}
  ...
  \begin{itemize}
    \uncover<2->{ \item Allstate has held three Kaggle competitions.  All three were won by algorithms incorporating gradient boosting as a fundamental component. }
    \uncover<3->{ \item In the 1990's insurance company's discovered that incorporating consumers credit information into pricing greatly increased the accuracy of prices, which revolutionized the industry.  Using a boosted model in place of a (generalized) linear model when setting prices gives roughly the same increase in predictive power. }
  \end{itemize}
\end{frame}
%
\begin{frame}
